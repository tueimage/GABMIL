...
class NystromAttention(nn.Module):
    ...
    def forward(self, x, mask = None, return_attn = False):
        ...
        attn1_, attn2_, attn3_ = map(lambda t: t.softmax(dim = -1), (attn1, attn2, attn3))
        attn2_ = moore_penrose_iter_pinv(attn2_, iters)
        out = (attn1_ @ attn2_) @ (attn3_ @ v)
        ...
        out = out[:, -n:]
        if return_attn:
            attn1_ = attn1_[:,:,:self.n_token] @ attn2_
            attn1_ = (attn1_ @ attn3_)
        
            return out, attn1_.mean(1)
...
class TransLayer(nn.Module):
    ...
    def forward(self, x):
        y, A = self.attn(self.norm(x), return_attn=True)
        x = x + y
...
class TransMIL(nn.Module):
    ...
    def forward(self, input, coords= None):
        ...
        # ---->Translayer x1
        h, _ = self.layer1(h)  # [B, N, 256]

        # ---->PPEG
        h = self.pos_layer(h, _H, _W)  # [B, N, 256]

        # ---->Translayer x2
        h, A = self.layer2(h)  # [B, N, 256]
